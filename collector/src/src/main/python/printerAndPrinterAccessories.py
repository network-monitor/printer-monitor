...
class Printer():
    ...
    def toInfluxFileFormat(self):
        row = "printer"
        row += (",agent_host=" + self.ip)
        row += (" hrDeviceDescr=" + self.description)
        row += (",sysUpTime=" + self.uptime)
        row += ",prtCoverStatus=4"
        row += ",prtCoverDescription=door closed"
        row += ("prtMarkerLifeCount=" + self.lifeCount)
        row += (",prtPowerCycleCount=" + self.powerOnCount)

        for supply in self.supplies:
            row += "\nprinter"
            row += (" agent_host=" + self.ip)
            row += (",prtMarkerSuppliesDescription=" + supply.description)
            row += (",prtMarkerSuppliesPercentLevel=" + supply.percentLevel)
            row += (",prtMarkerSuppliesLevel=" + supply.currentLevel)
            row += (",prtMarkerSuppliesMaxCapacity=" + supply.maxLevel)
            row += (",prtMarkerSuppliesType=" + supply.type)
            row += (",prtMarkerSuppliesUnit=" + supply.unit + "\n")
